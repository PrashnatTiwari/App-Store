...
class AppStore extends Component {
  ...
  changeTabItem = () => {
    const {ativeTabId} = this.state
    const filteredList = appsList.filter(
      eachItem => eachItem.category === ativeTabId,
    )
    return filteredList
  }
  ...
  getSearchResult = () => {
    ...
    const searchResult = appsList.filter(each =>
      each.appName.toLowerCase().includes(inputSearchText.toLowerCase()),
    )
    return searchResult
  }

  render() {
    const {inputSearchText, ativeTabId} = this.state
    const searchResult = this.getSearchResult()
    const filteredApps = this.changeTabItem(searchResult)
        ...
        <ul className="unordered-list-item">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              isActive={ativeTabId === eachItem.tabId}
            />
          ))}
        </ul>
